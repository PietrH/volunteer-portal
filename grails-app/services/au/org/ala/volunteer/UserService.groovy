...
class UserService {
    ...
    def getUserCounts(List<String> ineligibleUsers = []) {
        ...
        def users = User.executeQuery("""
            select new map(concat(coalesce(firstName, ''), ' ', coalesce(lastName, '')) as displayName, email as email, transcribedCount as transcribed, validatedCount as validated, (transcribedCount + validatedCount) as total, userId as userId, id as id)
            from User
